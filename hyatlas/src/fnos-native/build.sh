#!/bin/bash
# hyatlas fpk 打包层还原构建脚本
#
# 目录来源:
# - cmd/ config/ wizard/ manifest: 从原 fpk (sha256 3a45989ac3d0d3e7938608feca4dcc9efa9db54b09224582407f830a61955e5c)
#   解包原样还原
# - app.tgz: 定档自 2.0.1 首 fpk 包内 app.tgz, 字节原样, 不重打。
#   原因: 汉化/降权改动全部在 app.tgz 内 site-packages(hy_memory) 中,
#   而仓库 src/ 是纯上游 Python 树(无打包层、无改动), 从 src/ 无法复现
#   这些改动, 故将 app.tgz 定档为唯一应用包来源。
#
# checksum 注意:
# - manifest 内 checksum 字段 = md5(app.tgz)。本脚本用 md5sum 校验两者一致,
#   不一致则中止, 防止 app.tgz 被替换后 checksum 失配。
# - 当前值: 4b4f847f7c44a4047f023ad8c1595280。若将来更新 app.tgz, 需用
...
[ -f "$WORK/hyatlas.fpk" ] || { echo "ERROR: fnpack 未产出 fpk" >&2; exit 1; }
...
out = os.path.join(work, "hyatlas.fpk")
os.chdir(work)
...
# --- 4. 产物回移覆盖项目根 fpk ---
mv -f "$WORK/hyatlas.fpk" "$PROJ_ROOT/hyatlas.fpk"

echo "built: $PROJ_ROOT/hyatlas.fpk"
echo "size: $(stat -c '%s' "$PROJ_ROOT/hyatlas.fpk")"
sha256sum "$PROJ_ROOT/hyatlas.fpk"
