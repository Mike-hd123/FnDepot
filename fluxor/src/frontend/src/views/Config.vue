...
const isSmartCore = computed(() => String(stats.value.coreVersion || '').includes('-smart-'))
...
const handleUpgradeCore = async (channel?: string) => {
  ...
  try {
    // smart 内核走专用通道（后端会拒绝通用 /upgrade 以防降级回裸 alpha）
    let url = isSmartCore.value && !channel ? '/core/smart-update' : '/upgrade'
    if (channel) url += `?channel=${channel}`
    const resp = await apiFetch(url, { method: 'POST' })
    if (resp.ok) {
      // smart 通道：updated=false 表示已是最新，无需刷新内核
      if (url === '/core/smart-update') {
        try {
          const j = await resp.clone().json()
          if (j && j.updated === false) {
            globalStore.showToast(j.message || t('config.upgrade_already_latest'), 'warning')
            isUpgrading.value = false
            showUpgradeMenu.value = false
            return
          }
        } catch (_) { /* ignore parse error, proceed */ }
      }
              ...
              <div ref="upgradeMenuRef" class="relative flex w-full">
                ...
                >
                  <button
                    ...
                    @click="handleUpgradeCore()"
                    :disabled="isUpgrading || !coreStatus.running"
                    class="w-full text-left px-4 py-2 text-xs font-medium text-emerald-600 dark:text-emerald-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                  >
                    {{ t('config.upgrade_smart') }}
                  </button>
                  <template v-else>
                  <button
