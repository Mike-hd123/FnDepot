...
//   - 实例容器 IP = 192.168.5.50（WOC_LAN_IP env，默认 .50），微信备份同广播域。
//   - 面板在宿主机，经宿主 ipvl0 虚接口（.51 slave, parent=默认路由物理网卡）
//     访问容器 .50:3000（ipvlan 父↔子隔离：宿主不能直接 curl 子接口 IP）。
//     ipvl0 持久化由 install_callback/upgrade_callback 写 systemd 单元
//     (woc-ipvl0.service + /usr/local/bin/woc-ensure-ipvl0.sh) 负责。
//   - 反代 fetchContainerIP 只取 woc-lan 网卡 IP。
// 数据卷：WOC_DATA_DIR 非空 → 主机路径 bind (/vol2/1000/weixin/woc-data-<id>)；
...
function defaultRouteIface(): string | null {
  try {
    ...
    for (const line of data.split('\n').slice(1)) {
      ...
      const [iface, dest] = parts;
      if (dest === '00000000') return iface; // default 路由所在接口
    }
    return null;
  } catch {
    return null;
...
export async function ensureWocLan(containerName: string): Promise<void> {
  try {
    ...
    // 网络不存在 → 创建（parent 动态取默认路由物理网卡，别硬编码 eno1）
    const parent = defaultRouteIface() || 'eno1';
    appendPanelLog(
      'INFO',
      `woc-lan 网络不存在，自动创建（ipvlan l2, parent=${parent}, subnet 192.168.5.0/24）`,
    );
    try {
      await docker.createNetwork({
        Name: WOC_LAN_NET,
        Driver: 'ipvlan',
        Options: { parent, ipvlan_mode: 'l2' },
        IPAM: {
          Driver: 'default',
          Config: [{ Subnet: '192.168.5.0/24', Gateway: '192.168.5.1' }],
        },
      } as any);
    } catch (e: any) {
      appendPanelLog('WARN', `woc-lan 创建失败：${e?.message || e}`);
      return;
    }
  }
  try {
...
export async function runInstance(inst: Instance, opts?: { keepImage?: boolean }): Promise<void> {
  ...
  const dataDir = dataDirFor(inst);
  if (dataDir) {
    try {
      if (!existsSync(dataDir)) {
        mkdirSync(dataDir, { recursive: true });
        try { chownSync(dataDir, 1000, 1000); } catch { /* 非 root 忽略 */ }
      }
    } catch (e: any) {
  ...
  // ⚠️ v1.5.0 单网卡（woc-lan=ipvlan）：ipvlan 接口**不支持自定义 MAC**（docker 报
  // "ipvlan interfaces do not support custom mac address assignment"），此时必须跳过 MAC。
  // MAC 伪装仅对 bridge/macvlan 有意义。ipvlan 下容器 MAC 与宿主物理网卡共享。
  if (net) {
    const ep: any = {} as any;
...
> {
  ...
  for (const c of allContainers) {
    for (const m of c.Mounts || []) {
      // 兼容两种形态：命名卷 (Name=woc-data-*) 与 bind 主机路径 (Source=.../woc-data-*)
      if (typeof m.Name === 'string' && m.Name.startsWith('woc-data-')) containerRefs.add(m.Name);
      if (typeof m.Source === 'string') {
        const base = m.Source.split('/').pop() || '';
        if (base.startsWith('woc-data-')) containerRefs.add(base);
      }
    }
  }
  ...
  if (Array.isArray(Volumes)) {
    for (const v of Volumes) {
      if (typeof v?.Name !== 'string' || !v.Name.startsWith('woc-data-') || referenced.has(v.Name)) continue;
      out.push({
        name: v.Name,
...
// 兼容两种形态：命名卷（docker volume rm）或 WOC_DATA_DIR 下 bind 目录（rm -rf）。
export async function removeVolume(name: string): Promise<void> {
  const dataDir = (process.env.WOC_DATA_DIR || '').trim();
  if (dataDir && name.startsWith('woc-data-') && !name.includes('/')) {
    // bind 形态：name 为目录基名（woc-data-<id>），落在 WOC_DATA_DIR 下 → 删目录
    rmSync(`${dataDir}/${name}`, { recursive: true, force: true });
    return;
  }
...
export async function listOrphanContainers(
  ...
  for (const c of all) {
    ...
    // 兼容两种形态：命名卷 (Name=woc-data-*) 与 bind 主机路径 (Source=.../woc-data-*)
    const volName = (c.Mounts || []).map((m) =>
      typeof m.Name === 'string' && m.Name.startsWith('woc-data-')
        ? m.Name
        : (typeof m.Source === 'string' && (m.Source.split('/').pop() || '').startsWith('woc-data-')
            ? (m.Source.split('/').pop() || '') : undefined),
    ).find((x) => x !== undefined);
...
// 实例容器名 → IP 缓存（方案 A：反代按 IP 访问，不再依赖 /etc/hosts）。
// 缓存 2s 过期：容器重启/重建换 IP，2s 内自动重查自愈，无脏读残留风险。
const ipCache = new Map<string, { ip: string; ts: number }>();
const IP_TTL_MS = 2_000;

// 查某实例容器的 IP（v1.5.0 纯 ipvlan：实例只有 woc-lan 网卡）。
// 先取 woc-lan 网 IP；兜底任意网络首个非空 IP；查不到返回 null。
async function fetchContainerIP(containerName: string): Promise<string | null> {
  try {
    const info: any = await docker.getContainer(containerName).inspect();
    const nets: Record<string, any> = info?.NetworkSettings?.Networks || {};
    // 优先 woc-lan（同广播域业务网；面板经 ipvl0 可达）
    const lan = nets['woc-lan']?.IPAddress;
    if (lan) return lan;
    // 兜底：任意网络首个非空 IP
    for (const n of Object.values(nets)) {
      const ip = (n as any)?.IPAddress;
      if (ip) return ip;
    }
    return null;
  } catch {
    return null; // 容器不存在/已删 → null，调用方走容器名兜底或 502
  }
}

// 实例容器名（供反代构造 target）。
// 实时查 docker.sock 拿容器 IP（带 2s 缓存；查不到回退容器名兜底）。
export async function instanceTarget(inst: Instance): Promise<string> {
  const now = Date.now();
  const hit = ipCache.get(inst.containerName);
  if (hit && now - hit.ts < IP_TTL_MS) return `http://${hit.ip}:3000`;
  const ip = await fetchContainerIP(inst.containerName);
  if (ip) {
    ipCache.set(inst.containerName, { ip, ts: now });
    return `http://${ip}:3000`;
  }
  ipCache.delete(inst.containerName);
  return `http://${inst.containerName}:3000`; // 容器名兜底（Docker 内嵌 DNS）
}
