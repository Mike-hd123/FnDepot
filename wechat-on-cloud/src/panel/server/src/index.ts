...
app.addHook('onRequest', async (req, reply) => {
  if (SOCK_PATH) return; // unix socket 直连：本机专用，无 DNS-rebinding 风险
...
app.server.on('upgrade', (req: IncomingMessage, socket: Socket, head: Buffer) => {
  ...
  if (SOCK_PATH) {
    // unix socket 直连：本机专用，无 DNS-rebinding 风险，与 HTTP onRequest 短路一致
    // （fnOS 网关经 socket 反代时 Host=server.local 不在白名单，须放行）
  } else if (!isRequestHostAllowed(req.headers.host, req.headers['x-forwarded-host'], ALLOWED_HOSTS)) {
  ...
  // 方案 A：反代目标实时查 docker.sock 拿容器 IP（2s 缓存）。upgrade 回调同步 → 异步解析 target
  instanceTarget(inst).then((target) => {
    proxy.ws(req, socket, head, { target }, (err: any) => {
      appendInstanceLog(inst.id, `[vnc] 连接失败：${err?.message || err}`);
    });
  }).catch((e: any) => {
    appendInstanceLog(inst.id, `[vnc] 解析目标失败：${e?.message || e}`);
    socket.destroy();
  });
});
...
if (SOCK_PATH) {
  ...
  appendPanelLog('INFO', `面板启动 · 监听 unix:${SOCK_PATH}`);
} else {
...
try {
  ...
  const proxy = createServer(async (client) => {
    const upstream = connect({ path: SOCK_PATH });
