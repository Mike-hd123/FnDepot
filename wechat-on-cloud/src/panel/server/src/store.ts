...
export function createInstance(
  ...
  if (reuseVolumeName) {
    ...
    if (useBind && reuseVolumeName.includes('/')) {
      // bind 形态复用：调用方传的是完整路径 → 归一为基名
      volumeName = reuseVolumeName.split('/').pop() || reuseVolumeName;
    } else if (!useBind && reuseVolumeName.includes('/')) {
      // 调用方误传完整路径但当前非 bind 形态 → 也用基名
      volumeName = reuseVolumeName.split('/').pop() || reuseVolumeName;
    } else {
