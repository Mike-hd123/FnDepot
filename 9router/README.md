...
**本打包**: 基于上游 v0.5.59 构建的 fnOS 原生 fpk（不再依赖上游 `9router-fnos` 维护者的打包，改为 Mike 自打包）。
...
├── 9router-0.5.59-x86.fpk        # 打包产物
├── README.md
└── src/                          # 打包层源码存档（可复现构建）
    ├── build.sh                  # 一键构建脚本（clone 上游 v0.5.59 → standalone → fnpack）
...
```bash
cd src/
./build.sh 0.5.59 x86      # 产物输出到仓库根
```

构建脚本会: clone decolua/9router@v0.5.59 → npm ci（npmmirror 镜像）→ `NEXT_DIST_DIR=.next-cli-build npm run build` 产出 standalone → 拷贝 open-sse / src/mitm / sql.js / node-forge 运行时依赖 → 组装 fnOS 打包结构 → fnpack build。
